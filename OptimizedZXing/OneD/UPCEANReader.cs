...
namespace OptimizedZXing.OneD
{
    internal abstract class UPCEANReader : OneDReader
    {
        ...
        /// </returns>
        public override Result DecodeRow(int rowNumber, BitArray row)
        {
            var startGuardRange = FindStartGuardPattern(row);
            ...
            var points = new ResultPoint[]
            {
                new ResultPoint(left, rowNumber),
                new ResultPoint(right, rowNumber)
            };
            return new Result(resultString, null, points, format);
        }
        ...
        internal static bool DecodeDigit(BitArray row, int[] counters, int rowOffset, int[][] patterns, out int digit)
        {
            ...
            if (!RecordPattern(row, rowOffset, counters, counters.Length))
                return false;
