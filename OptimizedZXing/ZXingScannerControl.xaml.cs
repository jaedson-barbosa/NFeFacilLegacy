...
namespace OptimizedZXing
{
    public sealed partial class ZXingScannerControl : UserControl, IDisposable
    {
        ...
        public async void StartScanning (Action<Result> scanCallback, MobileBarcodeScanningOptions options = null)
        {
        ...
        public async Task StartScanningAsync(Action<Result> scanCallback, MobileBarcodeScanningOptions options)
        {
            if (stopping)
                return;
            ...
            VideoEncodingProperties previewResolution = null;
            // Loop through all supported sizes
            foreach (var sps in availableProperties)
            {
            ...
            {
                previewResolution = availableProperties.LastOrDefault();
            }

            if (previewResolution == null)
            {
                throw new Exception("No preview resolution available. Camera may be in use by another application.");
            }

            // Find the matching property based on the selection, again
            var chosenProp = availableProperties.FirstOrDefault(ap => ap.Width == previewResolution.Width && ap.Height == previewResolution.Height);
            ...
            timerPreview = new Timer(async (state) => {
                ...
                || (mediaCapture == null || mediaCapture.CameraStreamState != CameraStreamState.Streaming))
                {
        ...
        async Task<DeviceInformation> GetFilteredCameraOrDefaultAsync(MobileBarcodeScanningOptions options)
        {
            ...
            var selectedCamera = videoCaptureDevices.FirstOrDefault(vcd => vcd.EnclosureLocation != null
                && vcd.EnclosureLocation.Panel == Windows.Devices.Enumeration.Panel.Back);

            return selectedCamera ?? videoCaptureDevices.FirstOrDefault();
        }
